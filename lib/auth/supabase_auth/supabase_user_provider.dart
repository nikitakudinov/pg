...
class PlaygroundSupabaseUser extends BaseAuthUser {
  PlaygroundSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> playgroundSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = PlaygroundSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
